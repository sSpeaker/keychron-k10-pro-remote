/* Keychron K10 Pro ANSI RGB - Keymap with LED-based BT remote control
 *
 * Based on the default keymap. Adds led_update_user() hook to intercept
 * LED output reports from the host and decode BT commands.
 *
 * Build:
...
/* ── LED-based BT remote control hooks ─────────────────────────── */

bool led_update_user(led_t led_state) {
    /* Returns false to suppress LED port update for command bytes,
     * true for normal LED indicator updates (Caps Lock, Num Lock). */
    return led_bt_control_process(led_state);
}

void matrix_scan_user(void) {
    /* Timeout safety for the command state machine */
    led_bt_control_task();
}
